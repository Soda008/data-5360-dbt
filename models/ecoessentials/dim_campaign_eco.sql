...
with transactional_source as (
    select cast(campaign_id as varchar) as campaign_id, campaign_name, campaign_discount
    from {{ source('rds_source', 'PROMOTIONAL_CAMPAIGN') }}
),
s3_source as (
    select distinct cast(campaignid as varchar) as campaignid, campaignname
...
final as (
    ...
    full join s3_source s on t.campaign_id = s.campaignid
)
select {{ dbt_utils.generate_surrogate_key(['campaign_id']) }} as campaign_key, * from final